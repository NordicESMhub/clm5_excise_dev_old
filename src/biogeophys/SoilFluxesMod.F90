...
!Edit by Lei Cai--end
    ...
    associate(                                                                & 
	 ...
	 eflx_lateralheat_col    => energyflux_inst%eflx_lateralheat_col    , & ! Input:  [real(r8) (:,:) ]  col lateral heat flux heat flux (W/m**2) !KSA2019
...
!         errsoi_patch(p) =  errsoi_patch(p)+eflx_h2osfc_to_snow_col(c) 
         errsoi_patch(p) =  errsoi_patch(p)+eflx_h2osfc_to_snow_col(c) + sum(eflx_lateralheat_col(c,:)) !KSA2019 added lateral heat flux
