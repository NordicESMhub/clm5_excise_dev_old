...
CONTAINS 
  ...
  subroutine depvel_compute( bounds, &
    ...
    use landunit_varcon, only : istsoil, istsoil_li, istsoil_mi, istsoil_hi, istice_mec, istdlak, istwet   !KSA2019
    ...
    associate(                                                    & 
      ...
      pft_loop: do pi = bounds%begp,bounds%endp
         ...
         active: if (patch%active(pi)) then
            ...
            if ( lun%itype(l) /= istsoil .and. &  !KSA2019
	    lun%itype(l) /= istsoil_li .and. &	 
	    lun%itype(l) /= istsoil_mi .and. &	 
